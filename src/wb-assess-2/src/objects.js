...
function getNameUsingDestructuring(person) {
  ...
  // return person; //two failed return statements
...
function getPeopleByLocation(people, location) {
  newArr = [];
  /*check if people[2] == location if yes return to newArr*/
}
...
function translateToPirateTalk(phrase) {
 let words = phrase.split(` `); //splits at spaces
 let newArr = [];//array to push new words too
 for(let i = 0; i < words.length; i++){ //for let loop //loops whil i < words.length
  if(EN_PIRATE_LOOKUP[words[i]]){//Returns true or false //finds the key location using bracket notation for string or var at pos I and if true then run code \/
    newArr.push(EN_PIRATE_LOOKUP[words[i]]);//returns the value to newArr
  }
  else{
    newArr.push(words[i]);//if word does not exist at EN_PIRATE_LOOKUP[words[i]] then push just the word
  }
 }
...
function isBugAvailable(bug, month) {
  //checking inside of bug object to see if .month == month
}
